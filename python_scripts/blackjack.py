...
def call_card(cards, hand, score, num_cards=1):
    ...
    if 11 in hand and score > 21: # Replaces the Ace (value of 11) with an Ace (value of 1) to avoid going over 21, for both dealer and user
        hand.remove(11)
        hand.append(1)
        score = sum(hand)
    ...
    if user_plays == "y":
        ...
        while True:
            if your_score > 21:
                ...
                print(f"You've hit blackjack with a score of {your_score}!\n The dealer will now shuffle again")
                time.sleep(3)
                break

            elif dealer_score > 21:
                print(f"Dealer went bust! Their score is: {dealer_score}\n The dealer will now shuffle again")
                break
